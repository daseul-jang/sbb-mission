...
type SocialType = {
  provider: Provider;
  imgIcon?: React.ReactElement<HTMLImageElement>;
  style?: string;
...
export default function SocialRoundIcon({ socialType }: Props) {
  const type = social[socialType];
  const baseStyle =
    'w-11 h-11 items-center justify-center inline-flex rounded-full font-bold text-lg text-white hover:shadow-lg cursor-pointer transition ease-in duration-300';

  return <span className={`${baseStyle} ${type.style}`}>{type.svgIcon}</span>;
}
